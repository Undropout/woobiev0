...
export async function loadTier1b(container, username, matchID) {
  const answersSnap = await get(ref(db, `matches/${matchID}/tier1b`));
  const lettersSnap = await get(ref(db, `matches/${matchID}/tier1bLetters`));
  const all = answersSnap.val() || {};
  const letters = lettersSnap.val() || {};
  const partner = Object.keys(all).find(k => k !== username);

  if (!partner || !all[username] || !all[partner]) {
    container.innerHTML += '<p>Tier 1b responses are still pending...</p>';
    return;
  }

  container.innerHTML += '<h3>🧭 Tier 1b</h3>';
  questions.forEach((q, i) => {
    container.innerHTML += `
      <details><summary><strong>${q}</strong></summary>
      <p><strong>You:</strong> ${all[username][i]?.value || ''}</p>
      <p><strong>Your Match:</strong> ${all[partner][i]?.value || ''}</p>
      </details>`;
  });

  container.innerHTML += '<h3>💌 Optional Letter</h3>';
  container.innerHTML += `
    <details><summary>What you sent</summary><p>${letters[username] || '(no message)'}</p></details>
    <details><summary>What your match sent</summary><p>${letters[partner] || '(no message)'}</p></details>
  `;
}
